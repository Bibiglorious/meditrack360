...
with DAG(
    ...
    upload_api_to_bronze = PythonOperator(
        task_id="extract_api_to_bronze",
        python_callable=extract_lab_results_to_bronze,
        op_kwargs={
            "bucket_name": BUCKET_NAME,
            "aws_conn_id": AWS_CONN_ID,
            "local_bronze_dir": "/opt/airflow/data/bronze/api",   # 👈 this matters
            "s3_prefix": "bronze/api",                             # 👈 folder in S3
            "replace": False,
        },
    )

    # run API upload AFTER lab_results
    upload_lab_results_to_bronze >> upload_api_to_bronze